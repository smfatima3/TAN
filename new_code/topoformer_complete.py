...
class TopologicalAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, topo_features: torch.Tensor,
        ...
        if mask is not None:
            filler = torch.finfo(scores.dtype).min
            scores = scores.masked_fill(mask.unsqueeze(1).unsqueeze(2) == 0, filler)
...
class TopoformerLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        token_out, _ = self.token_attention(x, x, x, key_padding_mask=key_padding_mask)
